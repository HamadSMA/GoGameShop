...
app.InitializeDb();      // Run migrations & seed data
...
- Migrations are applied on startup via `dbContext.Database.Migrate()` in `DataExtensions.cs`.
...
- `SaveChanges()` commits the inserts to the database.
...
**Update pattern (PUT):**
```csharp
var game = dbContext.Games.Find(id);  // 1. Find the existing game
...
dbContext.SaveChanges();              // 3. EF Core detects changes and issues UPDATE SQL
return Results.NoContent();           // 4. Return 204 — success, nothing to return
```

EF Core "tracks" the entity retrieved by `Find()`. When you mutate its properties and call `SaveChanges()`, EF Core automatically generates the SQL `UPDATE` statement.
...
No entity is loaded into memory — it's a direct, efficient delete.
