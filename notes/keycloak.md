...
The realm exists, but it's empty: there's no one to log in. A **user** in Keycloak is the identity that logs in and receives a JWT. Keycloak stores their credentials, profile attributes, role assignments, and session state; the application never sees any of this directly, only the claims that end up in the JWT.
...
3. Repeat for any other roles (e.g., none needed yet, since authenticated users without `Admin` are treated as regular customers)
...
The API's `UserAccess` policy calls `RequireClaim(ClaimTypes.Scope, "gogameshop_api.all")`, expecting a `scope` claim whose value is exactly `"gogameshop_api.all"`. But the access token Keycloak issues looks like this:
...
The custom `KeycloakClaimsTransformer` (covered in [notes/auth.md](auth.md)) runs once per token validation, splits the `scope` claim on spaces, and re-adds one `Claim(ClaimTypes.Scope, ...)` per individual scope. After it runs, the principal carries:
