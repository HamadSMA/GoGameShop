...
Running the same application on a developer laptop, a CI runner, and a production server without "it works on my machine" surprises requires freezing the environment alongside the code. Docker packages an application and everything it needs to run (runtime, libraries, system tools, configuration) into a single unit called a **container**. The container behaves the same on any machine that has Docker installed, regardless of the host operating system.
...
Images live in a **registry**, by default Docker Hub (`hub.docker.com`). The `docker pull` command downloads an image to your local machine so it can be used to start containers. You don't usually need to pull manually, since `docker run` pulls automatically if the image isn't already present.
...
An image is a frozen filesystem; nothing happens until you turn it into a live process. `docker run` creates a new container from an image and starts it. Flags control its lifecycle (`-d` to detach, `--rm` for one-off cleanup) and its identity (`--name` for a stable handle instead of a random one like `quirky_einstein`).
...
A container is a running process with its own filesystem and process tree, but you can't `cd` into it from the host or attach a debugger directly. `docker exec` runs a command inside a container that's already running. Combined with an interactive shell, it gives you a terminal session inside the container's environment without restarting it.

```
docker exec -it nginx-test /bin/bash
```

`exec` runs a command in an existing container, which is different from `docker run` (which creates a new one).
...
`/bin/bash` is the program to run, relative to the container's filesystem (Linux). Debian-based images like the default `nginx` have bash; Alpine-based images (`nginx:alpine`) only have `/bin/sh`.
...
Long `docker run` commands with many flags become hard to remember, share, and version-control. Real apps usually need *several* containers (API + database + cache) wired together, and orchestrating that by hand is fragile and impossible to onboard a teammate to.
...
- Maps host `8080` to container `80` (browse `http://localhost:8080`)
